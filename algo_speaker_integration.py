...
class AlgoSpeakerAPI:
    ...
    def _generate_hmac(self, method, uri, content_md5="", content_type="application/json", timestamp=None, nonce=None):
    ...
    def _get_headers(self, method, uri, payload=None):
        ...
        if payload:
            ...
            content_md5 = hashlib.md5(json.dumps(payload, separators=(',', ':')).encode('utf-8')).hexdigest()
    ...
    def stop_tone(self):
        ...
        try:
            response = requests.post(
                f"{self.base_url}/api/controls/tone/stop",
                headers=headers,
                timeout=10
            )
            return response.status_code == 200, response.text
    ...
    def stop_strobe(self):
        ...
        try:
            response = requests.post(
                f"{self.base_url}/api/controls/strobe/stop",
                headers=headers,
                timeout=10
            )
            return response.status_code == 200, response.text
    ...
    def get_device_info(self):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/settings/device.info",
                headers=headers,
                timeout=10
            )
